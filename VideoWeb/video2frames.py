    ...
    if only_output_video_info:
        ...
        while success and count < len(extract_time_points):
            ...
            if success:
                ...
                filename = os.path.join(pathOut, "{}.jpg".format(output_prefix, count + 1))
                param = [int(cv2.IMWRITE_JPEG_QUALITY), jpg_quality]
                h, w, rgb = image.shape
                if h*16/9==w:
                    pass
                elif h*16/9 > w:
                    image = image[0:int(w*9/16), w]
                else:
                    image = image[h, int(h*16/9)]

                # cv2.imwrite(os.path.join(pathOut, "{}.jpg".format(output_prefix, count + 1)), image,
...
# pathIn = '../videos/res.mp4'
# pathOut = '../videos/'
# video2frames(pathIn, pathOut, output_prefix='res', extract_time_points=(10,20,30,40))